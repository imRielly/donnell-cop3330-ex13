...
/**
 ...
 Simple interest is something you use only when making a quick guess. Most investments
 use a compound interest formula, which will be much more accurate.
...
public class App 
{
    public static void main( String[] args )
    {
        ...
        System.out.print("What is the number of times the interest compounds a year? ");
        int compound = in.nextInt();

        //A = P(1 + r/n)^(n*t)
        //A = P(1 + a)^(b)
        double a = (rate/100.00)/compound;
        double b = compound*years;
        double total = Math.ceil(principal*(Math.pow(1.00 + a,b)));

        System.out.println(money.format(principal) + " invested at " + rate + "% for " + years + " years compounded " + compound + " times per year is " + money.format(total) + ".");
    }
}
